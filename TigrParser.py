from TIGr import AbstractParser, AbstractParserBuilder
import re
import json
...
class TigrParserOne(AbstractParserBuilder):
    def __init__(self, new_drawer, exception_handler):
        super().__init__(new_drawer)
        self.exception = exception_handler
        self.draw_methods = {
            'select_pen': self.the_parser.drawer.select_pen,
            'pen_down': self.the_parser.drawer.pen_down,
            'pen_up': self.the_parser.drawer.pen_up,
            'go_along': self.the_parser.drawer.go_along,
            'go_down': self.the_parser.drawer.go_down,
            'draw_line': self.the_parser.drawer.draw_line,
        }
        ...
        try:
            ...
            self.exception.display_and_exit(e)

    def parse(self, raw_source):
        self.the_parser.source = _handle_source(raw_source)
        for line_number in range(0, len(self.the_parser.source) - 1):
    ...
    def _find_match(self, line_number):
        trimmed_line = self.the_parser.source[line_number].strip()
        if not trimmed_line:
            return None
